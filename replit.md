...
- `feed_reels(p_cursor, p_limit, p_tab, p_tag, p_ticker, p_q, p_user)` — the single read-side feed query. Tabs: `'fy'` (chronological), `'following'` (joined to `follows`), `'trending'` (engagement-weighted in last 24h). Filters: exact tag match, exact symbol match, exact author-username match (`p_user` — fed by the `@user` search prefix), substring search across thesis + tag values. Returns `{rows, next_cursor}` — each row carries the author profile (incl. `win_rate` + `closed_n`), tag list with pin coords, engagement counts (`likes/saves/shares/views/mirrors`), `paper_trade_qty` + `paper_trade_side` + `paper_trade_entry` (server-of-truth size and **fill price** used by the live-PnL badge so it matches the trade the publisher actually opened — `paper_trade_entry` is the live quote captured at publish time and may differ from `entry` when the user set a limit), and viewer state (`liked/saved/mirrored/following`).
